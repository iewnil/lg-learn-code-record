...
function ReactDOMBlockingRoot(
  ...
  // tag => 0 => legacyRoot
  // 实例的 _internalRoot 回到前面，其实就是：
  // 1. fiberRoot 对象的值
  // 2. container._reactRootContainer 的值
...
/**
 * 通过 new ReactDOMBlockingRoot创建一个 LegacyRoot 实例  
 * @param {*} container 
 * @param {*} options 
 * @returns  new ReactDOMBlockingRoot(container, LegacyRoot, options)
 */
export function createLegacyRoot(
  ...
  /**
   * LegacyRoot 是常量 0，表示此次调用是 ReactDOM.render 开启的
   */
 ...
 *    react 内部会找到注释节点的父级，通过调用父级元素的 insetBefore 方法，将 clement 插入到注释节点的前面
 * @param {*} node 
 * @returns 
 */
