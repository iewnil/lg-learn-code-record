    ...
    case ClassComponent: {
      if (finishedWork.effectTag & Snapshot) {
        if (current !== null) {
          ...
          // 开始执行类组件的 getSnapshotBeforeUpdate 生命周期函数
          // 在组件更新前捕获一些 DOM 信息
          // 返回自定的值或 null，统称为 snapshot
          ...
          // 将 snapshot 存进去（在类组件的 componentDidUpdate 的第三个参数中要获取到这个）
...
function commitPlacement(finishedWork: Fiber): void {
  ...
  let parent;
  // 是否为渲染容器
  // 渲染容器和普通 react 元素的主要区别在于：是否需要特殊处理注释节点
  let isContainer;
  // 获取父级 DOM 节点对象
  // 但是，初始渲染时 rootFiber 对象中的 stateNode 存储的是 FiberRoot
  ...
  // 渲染容器
  if (isContainer) {
    // 向父节点中 追加节点 或者 将子节点插入到 before 节点的前面
    insertOrAppendPlacementNodeIntoContainer(finishedWork, before, parent);
  } else {
    // 非渲染容器
    // 向父节点中追加节点 或者 将子节点插入到 before 节点的 前面
    insertOrAppendPlacementNode(finishedWork, before, parent);
  }
}

/**
 * 向容器中追加 或 插入到某一个节点的前面
 * @param {*} node 
 * @param {*} before 
 * @param {*} parent 
 */
function insertOrAppendPlacementNodeIntoContainer(
  ...
  const {tag} = node;
  // 如果待插入的节点是一个 DOM 元素 或者文本的话
  // 比如 组件 fiber -> false div -> true
  ...
  if (isHost || (enableFundamentalAPI && tag === FundamentalComponent)) {
    ...
    // 如果是组件节点， 比如 classComponent，则找它的第一个子节点（DOM元素）
    // 进行插入操作
