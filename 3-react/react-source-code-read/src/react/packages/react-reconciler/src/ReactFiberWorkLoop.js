...
/**
 * 核心：判断任务是否为同步任务，若是，则调用执行同步任务入口
 * @param {*} fiber 初始渲染时为 rootFiber（即 <div id="root"></div> 对应的 fiber对象）
 * @param {*} expirationTime 任务过期时间，初始渲染时为 1073741823
 * @returns 
 */
export function scheduleUpdateOnFiber(
  ...
  /**
   * 检查是否是嵌套任务
   * 在 componentWillUpdate 或者 componentDidUpdate 生命周期函数中重复调用 setState 方法时
   * 可能会出现这种情况，React 限制了嵌套更新的数量（NESTED_UPDATE_LIMIT=50），以防止无限嵌套
   */
  ...
  /**
   * 判断是否有高优先级的任务打断当前正在执行的任务
   * 初始渲染时，内部条件不成立，内部代码不会执行
   */
  ...
  // 获取当前调度任务的优先级，数值类型，从 90-99 开始，数值越大，优先级越高
  // 97 普通优先级任务
  const priorityLevel = getCurrentPriorityLevel();

  // 根据过期时间（1073741823）判断任务是否是同步任务 Sync = MAX_SIGNED_31_BIT_INT = 1073741823
 ...
 * 这是不经过调度程序的同步任务的入口点
 * 正式的进入 render 阶段
 * 构建 workInProgress Fiber 树（即，为每一个 React 元素构建 fiber 对象）
 * @param {*} root current fiber 树中的 fibeRoot
 * @returns 
 */
function performSyncWorkOnRoot(root) {
  // Check if there's expired work on this root. Otherwise, render at Sync.
  // 检查是否有过期的任务，如果没有则为 0（初始化渲染肯定没有）
  const lastExpiredTime = root.lastExpiredTime;
  // NoWork 值为 0
  // 如果有过期任务，则过期时间设置为 lastExpiredTime，
  // 否则，设置为 Sync=1073741823（因为初始渲染肯定没有）
  ...
  // and prepare a fresh one. Otherwise we'll continue where we left off.
  /**
   * root 与 workInProgressRoot 不相等（即，当前 current fiber 树的 fiberRoot 不等于 workInProgress Fiber树的 fiberRoot）
   * 说明 workInProgressRoot 不存在，即还没有构建 workInProgress Fiber 树
   * workInProgressRoot 为全局变量，默认为 null，初始渲染时为 null
   * expirationTime => 1073741823
   * renderExpirationTime => 0
   */
  if (root !== workInProgressRoot || expirationTime !== renderExpirationTime) {
    // 创建 workInProgress Fiber 树的 fiberRoot 对象，以及 rootFiber
    prepareFreshStack(root, expirationTime);
    // 初始渲染不执行，内部条件判断不成立
    startWorkOnPendingInteractions(root, expirationTime);
  }
  ...
  if (workInProgress !== null) {
    ...
    do {
      try {
        // 开启一个循环，以同步的方式开始构建 workInProgress Fiber 树中的每一个 fiber 节点
        workLoopSync();
        break;
      } catch (thrownValue) {
        handleError(root, thrownValue);
    ...
    if (workInProgress !== null) {
      ...
      stopFinishedWorkLoopTimer();
      // 将构建好的新 fiber 对象（即 workInProgres Fiber树）存储在 finishedWork 属性中
...
function finishSyncRender(root) {
  // Set this to null to indicate there's no in-progress render.
  // 销毁 wornInPrgressRoot 上存储的旧的 workInProgress Fiber 树引用
  workInProgressRoot = null;
  // 进入 commit 阶段
  commitRoot(root);
}
...
/**
 * 创建 workInProgress Fiber 树的 fiberRoot 对象，以及 rootFiber
 * @param {*} root  current Fiber树的 fiberRoot
 * @param {*} expirationTime 
 */
function prepareFreshStack(root, expirationTime) {
  // 为 fiberRoot 对象添加 finishedWork 属性
  // finishedWork 用于存储：在 render阶段完成后，构建的待提交的 fiber 对象
  ...
  // 设置 workInProgress Fiber 树的 fiberRoot 对象（workInProgressRoot）为 current Fiber树的 fiberRoot
...
/**
 * 构建某个节点 fiber 对象，会被调用多次，一直到没有 fiber需要构建
 * @param {*} unitOfWork 第一次调用时，一般就是 workInProgress Fiber 树中的 rootFiber
 * @returns 
 */
function performUnitOfWork(unitOfWork: Fiber): Fiber | null {
  ...
  // 获取到 current Fiber 树的 rootFiber
  ...
  if (enableProfilerTimer && (unitOfWork.mode & ProfileMode) !== NoMode) {
    ...
    // 从父到子（即，上到下），构建 workInProgress 树的某个 fiber 对象并返回
    next = beginWork(current, unitOfWork, renderExpirationTime);
  }
  ...
  // 记录每次更新的 props（相对于下一次，就是旧 props）
  unitOfWork.memoizedProps = unitOfWork.pendingProps;

  // next为null，说明左侧树构建 fiber 已经到底了
  if (next === null) {
    // If this doesn't spawn new work, complete the current work.
    /**
     * 继续往上回退，构建兄弟节点 fiber 对象并返回
     */
 ...
 * 2. 创建 fiber 对应的真实 DOM 对象，并将其添加到 stateNode 属性中
      ...
      if (
        ...
        if (effectTag > PerformedWork) {
          ...
          if (returnFiber.lastEffect !== null) {
            ...
            // head1：第一次链尾不存在，则说明 workInProgress 是链头，标记链头
            returnFiber.firstEffect = workInProgress;
          }

          // tail1：标记链尾，发生在“串联同级”之间的 fiber 时
...
function commitRoot(root) {
  ...
  // 使用最高优先级 99（ImmediatePriority）执行当前任务，因为 commit 阶段不允许被打断，而最高优先级任务不会被打断
  runWithPriority(
    ImmediatePriority,
...
function commitBeforeMutationEffects() {
  // 循环收集的 effect 链表，是从 root.finishedWork 里获取的，里面存储的是要执行 DOM 操作的 fiber对象
  // 从 firestEffect 一直到 lastEffect 中每一个都是需要 commit 的 fiber 对象
  while (nextEffect !== null) {
    // 要执行的 DOM 操作对应的 tag
    const effectTag = nextEffect.effectTag;
    
    // 如果 fiber 对象中有 Snapshot 这个 effectTag 的话（只在更新阶段有，初始渲染没有）
...
function commitMutationEffects(root: FiberRoot, renderPriorityLevel) {
  ...
  while (nextEffect !== null) {
    ...
    switch (primaryEffectTag) {
      // 针对该 fiber 节点对应的 DOM 节点及其子节点，进行插入操作
      case Placement: {
