 ...
 * @param {*} container fiberRoot 对象（要注意：不是 dom 对象了）
 * @param {*} parentComponent 父组件，初始渲染为 null
 * @param {*} callback 渲染完成执行的回调函数
 * @returns expirationTime 任务过期时间
 */
export function updateContainer(
  ...
  // container 为 fiberRoot
  // 获取 rootFiber（即 <div id="root"></div> 对应的 fiber对象）
  const current = container.current;
  // 获取当前距离 react 应用初始化的时间 1073741823
   ...
   * 初始化渲染时，任务同步执行不涉及被打断的问题，过期时间被设置成了 1073741823，这个数值表示当前任务为同步任务
   */
  ...
  // 初始渲染时，parentComponent 为 null，所以返回的 context 为 emptyContextObject = {} 空对象
  const context = getContextForSubtree(parentComponent);
  // 初始渲染时，container.context 为空
  if (container.context === null) {
    // 设置 container.context
...
export function getPublicRootInstance(
  ...
  // 获取到 rootFiber（即 <div id="root"></div> 对应的 fiber）
  ...
  switch (containerFiber.child.tag) {
    // 返回第一个子元素的 真实 DOM 对象 或 实例
    case HostComponent:
      // 此方法接收什么，返回什么，所以本质上还是 containerFiber.child.stateNode
