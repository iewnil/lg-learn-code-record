...
/**
 * 
 * @param {Boolean} shouldTrackSideEffects 是否给 fiber 对象添加 effectTag，true 添加，false 不添加
 *                                         （对初始化渲染来说，只有根组件需要添加，其他不需要，防止过多的 DOM 操作） 
 * @returns 
 */
function ChildReconciler(shouldTrackSideEffects) {
  ...
  /**
   * 构建单个子级 fiber
   * @param {*} returnFiber 其实是 workInProgress Fiber 树中的 rootFiber
   * @param {*} currentFirstChild 初始化渲染为 null
   * @param {*} element 上面传来的 element（其实就是子 vdom）
  ...
  function reconcileSingleElement(
    ...
    if (element.type === REACT_FRAGMENT_TYPE) {
      ...
      // 根据 ReactElement（vdom） 创建 fiber 对象
      const created = createFiberFromElement(
        element,
        // mode 用来表示当前组件下的所有子组件，要用何种渲染模式，继承自父级
        // 0 同步渲染模式
        // 100 异步渲染模式
        returnFiber.mode,
        expirationTime,
   ...
   * @param {*} returnFiber 其实是 workInProgress Fiber 树中的 rootFiber
   * @param {*} currentFirstChild 初始化渲染为 null
   * @param {*} newChild 上面传来的 element（其实就是子 vdom）
   * @param {*} expirationTime 
   * @returns 
   */
  function reconcileChildFibers(
    ...
    const isUnkeyedTopLevelFragment =
      ...
      newChild.key === null;
    // 如果是占位组件，则不处理，将其子元素赋值给 newChild
    if (isUnkeyedTopLevelFragment) {
      newChild = newChild.props.children;
    }

    // Handle object types
    // 判断子 vdom 是否为对象，若是，则说明就一个子元素
