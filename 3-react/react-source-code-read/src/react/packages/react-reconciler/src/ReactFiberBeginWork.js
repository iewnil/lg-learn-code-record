...
/**
 * 构建 workInProgress fiber 树中某 fiber 的 子级 Fiber 对象
 * @param {*} current current Fiber 树中的 rootFiber
 * @param {*} workInProgress workInProgress Fiber 树中的 rootFiber
 * @param {*} nextChildren  上面传来的 element
...
export function reconcileChildren(
  ...
  /**
   * 为什么要传递 current？
   * 因为 current 代表了当前页面显示的内容，对于下一次更新，也就是 旧 fiber，更新需要新旧 fiber 比对 
   * 初始化渲染时，current 为 null 
   */
  if (current === null) {
...
/**
 * 构建 workInProgress fiber 树的 rootFiber 的 fiber 对象 
 * @param {*} current current Fiber 树中的 rootFiber
 * @param {*} workInProgress workInProgress Fiber 树中的 rootFiber
...
function updateHostRoot(current, workInProgress, renderExpirationTime) {
  ...
  // 获取最新的 props （初始化渲染时为 null）
  const nextProps = workInProgress.pendingProps;
  // 获取当前（相对于最新的就是上一次）的 state（初始化渲染时为 null）
  const prevState = workInProgress.memoizedState;
  // 获取当前（上一次）渲染使用的 children（初始化渲染时为 null）
  const prevChildren = prevState !== null ? prevState.element : null;

  // 浅复制更新队列，防止引用属性互相影响（workInProgress.updateQueue 浅拷贝自 current.updateQueue）
  cloneUpdateQueue(current, workInProgress);
  // 获取 updateQueue.payload 并赋值到 workInProgress.memoizedState
  //（要渲染更新的内容就是 payload 中的 element，即 rootFiber 的子元素） 
  ...
  /**
   * 判断前后两个 Children 是否相同
   * 初始渲染时，preChildren => null  nextChildren => <App/>，所以不相同
   */
  // 尝试输出 nextChildren和 prevChildren
  ...
  // 返回rootFiber 的子 fiber 对象，下一次就是构建这个子 fiber 对象
  return workInProgress.child;
}
...
/**
 * 从父到子（即，上到下），根据 workInProgress 某个初始 fiber 对象tag类型，执行对应的具体构建方法，构建完整的 fiber对象并返回
 * @param {*} current current Fiber 树中的 fiber（第一个是 rootFiber）
 * @param {*} workInProgress workInProgress Fiber 树中的 fiber（第一个是 rootFiber）
 * @param {*} renderExpirationTime 
 * @returns 
 */
function beginWork(
  ...
  /**
   * 根据当前 fiber Tag 决定如何构建起子级 fiber 对象
   * （tag: shared/ReactWorkTags.js）
   */
  switch (workInProgress.tag) {
    // 2
    // 函数组件“第一次”被渲染时，tag为2
